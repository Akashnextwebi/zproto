...
public class Userlogins
{
    ...
    public static Userlogins CheckUserByEmail(SqlConnection conZP, string eMail)
    {
        Userlogins details = new Userlogins();
        try
        {
            DataTable dt = new DataTable();
            using (SqlCommand cmd = new SqlCommand("select FirstName,UserGuid,Status,EmailId from Customers where EmailId=@EmailId  and Status!='Deleted'", conZP))
            {
    ...
    public static int CreateUser(SqlConnection conZP, Userlogins Userlogins)
    {
        int x = 0;
        try
        {
            SqlCommand cmd = new SqlCommand("insert into Customers (UserGuid,Gender,FirstName,LastName,EmailId,ContactNo,Pwd,DOB,Status,Subscribed,ForgotId,AddedOn,AddedIp) values(@UserGuid,@Gender,@FirstName,@LastName,@EmailId,@ContactNo,@Pwd,@DOB,@Status,@Subscribed,@ForgotId,@AddedOn,@AddedIp)", conZP);
    ...
    public static Userlogins UserLoginWithEmail(SqlConnection conZP, string uid, string pwd)
    {
        Userlogins result = new Userlogins();
        try
        {
            DataTable dt = new DataTable();
            SqlCommand cmd = new SqlCommand("select Pwd,UserGuid,Status,FirstName from Customers where EmailId=@EmailId and Pwd=@Pwd and Status != 'Deleted'", conZP);
    ...
    public static Userlogins UserLogin(SqlConnection conZP, string uid, string pwd)
    {
        Userlogins result = new Userlogins();
        try
        {
    ...
    public static int UpdateAsVerifed(SqlConnection conZP, string uGuid)
    {
        int x = 0;
        try
        {
            SqlCommand cmd1 = new SqlCommand("Update Customers Set Status='Verified' Where UserGuid=@UserGuid", conZP);
