...
where Status='Active') x where RowNo > " + (6 * (cPage - 1));
    ...
    public static int InsertBlogDetails(SqlConnection conGV, BlogDetails cat)
    {
        ...
        try
        {
            string query = "Insert Into BlogDetails (ThumbImage,BlogImage,BlogTitle,BlogUrl,PostedBy,PageTitle,MetaKeys,MetaDesc,FullDesc,PostedOn,AddedOn,AddedBy,AddedIP,Status,DisplayHome) values" +
                           "(@ThumbImage,@BlogImage,@BlogTitle,@BlogUrl,@PostedBy,@PageTitle,@MetaKeys,@MetaDesc,@FullDesc,@PostedOn,@AddedOn,@AddedBy,@AddedIP,@Status,@DisplayHome)";
    ...
    public static int UpdateBlogDetails(SqlConnection conGV, BlogDetails cat)
    {
        ...
        try
        {
            string query = "Update BlogDetails Set DisplayHome=@DisplayHome,ThumbImage=@ThumbImage,BlogImage=@BlogImage,PostedOn = @PostedOn,PostedBy=@PostedBy,BlogTitle=@BlogTitle,BlogUrl=@BlogUrl,PageTitle=@PageTitle,MetaKeys=@MetaKeys,MetaDesc=@MetaDesc,FullDesc=@FullDesc,AddedOn=@AddedOn,AddedBy=@AddedBy,AddedIP=@AddedIP Where Id=@Id ";
            using (SqlCommand cmd = new SqlCommand(query, conGV))
            {
                ...
                cmd.Parameters.AddWithValue("@AddedOn", SqlDbType.DateTime).Value = cat.AddedOn;
                cmd.Parameters.AddWithValue("@AddedBy", SqlDbType.NVarChar).Value = cat.AddedBy;
                cmd.Parameters.AddWithValue("@DisplayHome", SqlDbType.NVarChar).Value = cat.AddedBy;
                cmd.Parameters.AddWithValue("@AddedIP", SqlDbType.NVarChar).Value = cat.AddedIP;
                cmd.Parameters.AddWithValue("@Status", SqlDbType.NVarChar).Value = "Active";
                conGV.Open();
                result = cmd.ExecuteNonQuery();
