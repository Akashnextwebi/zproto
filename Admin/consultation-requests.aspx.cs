...
public partial class Admin_consultation_requests : System.Web.UI.Page
{
    ...
    public void GetAllContactRequests()
    {
        try
        {
            List<ConsultationRequests> lcs = ConsultationRequests.GetAllConsultationRequests(conZP);
            int i = 0;
            foreach (ConsultationRequests pro in lcs)
            {
                strRequests += @"<tr>   
                                <td>" + (i + 1) + @"</td>                                           
                                <td>" + pro.FirstName + @"</td>
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conZP = new SqlConnection(ConfigurationManager.ConnectionStrings["conZP"].ConnectionString);
            ConsultationRequests BD = new ConsultationRequests();
            BD.Id = Convert.ToInt32(id);
            int exec = ConsultationRequests.DeleteConsultationRequests(conZP, BD);
            if (exec > 0)
            {
                x = "Success";
            }
    ...
    public static string GetMessage(string id)
    {
        ...
        {
            SqlConnection conZP = new SqlConnection(ConfigurationManager.ConnectionStrings["conZP"].ConnectionString);
            message = ConsultationRequests.GetMessageById(conZP, id);
        }
        catch (Exception ex)
        {
