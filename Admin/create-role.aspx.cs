...
public partial class Admin_create_role : System.Web.UI.Page
{
            ...
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    if (CreateUser.CheckAccess(conZP, pageName, "Edit", Request.Cookies["zp_aid"].Value))
                    {
                        ...
                        ca.Status = "Active";
                        int result = UserRoles.UpdateUserRoles(conZP, ca);
                        if (result > 0)
                        {
                            GetAllRoles();
                            GetRoles();
                ...
                {
                    if (CreateUser.CheckAccess(conZP, pageName, "Add", Request.Cookies["zp_aid"].Value))
                    {
                        ca.RoleName = txtName.Text;
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
                        int result = UserRoles.InsertUserRoles(conZP, ca);
                        if (result > 0)
                        {
                            txtName.Text = "";
                            GetAllRoles();
    ...
    public void GetAllRoles()
    {
        ...
        {
            strUserRoless = "";
            List<UserRoles> cas = UserRoles.GetAllUserRoles(conZP).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conZP = new SqlConnection(ConfigurationManager.ConnectionStrings["conZP"].ConnectionString);
            if (CreateUser.CheckAccess(conZP, "create-role.aspx", "Delete", HttpContext.Current.Request.Cookies["zp_aid"].Value))
            {
                UserRoles cat = new UserRoles();
                cat.Id = Convert.ToInt32(id);
                cat.UpdatedOn = TimeStamps.UTCTime();
                int exec = UserRoles.DeleteUserRoles(conZP, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
