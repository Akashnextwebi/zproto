...
public partial class Admin_manage_role_access : System.Web.UI.Page
{
    ...
    public void GetRoleDetails()
    {
        try
        {
            List<UserRoles> comps = UserRoles.GetAllUserRoles(conZP).Where(x => x.Id == Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
    ...
    public void GetUserAccess()
    {
        try
        {
            List<UserRolesAccess> users = UserRoles.GetAllUserRoles(conZP, Request.QueryString["id"]).OrderBy(x => x.GOrderBy).ToList();
            int i = 0;
...
<div class='form-check form-switch changeAccess' id='v_" + rl.PageId + @"'  data-id='" + rl.PageId + @"'>
    ...
    public static string ChangeStatus(string pid, string rid, string v, string a, string e, string d)
    {
        ...
        try
        {
            ...
            SqlConnection conZP = new SqlConnection(ConfigurationManager.ConnectionStrings["conZP"].ConnectionString);
            List<UserRolesAccess> users = UserRoles.GetAllUserRolesCheck(conZP, rid, pid);
            UserRolesAccess ua = new UserRolesAccess();
            if (users.Count > 0)
            {
                if (CreateUser.CheckAccess(conZP, "manage-role-access.aspx", "Edit", HttpContext.Current.Request.Cookies["zp_aid"].Value))
                {
                    ua.PageId = pid;
                    ua.RoleId = rid;
                    ...
                    int exec = UserRoles.UpdateRoleAccess(conZP, ua);
                    if (exec > 0)
                    {
                        x = "Success";
                    }
            ...
            {
                if (CreateUser.CheckAccess(conZP, "manage-role-access.aspx", "Add", HttpContext.Current.Request.Cookies["zp_aid"].Value))
                {
                    ua.PageId = pid;
                    ua.RoleId = rid;
                    ua.AddAccess = a == "Y" ? "1" : "0";
                    ...
                    ua.Status = "Active";
                    ua.PageGroupId = "";
                    int exec = UserRoles.InsertToRoleAccess(conZP, ua);
                    if (exec > 0)
                    {
                        x = "Success";
                    }
