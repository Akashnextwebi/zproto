...
public partial class Admin_manufacturing_partner : System.Web.UI.Page
{
    ...
    public void GetAllContactRequests()
    {
        try
        {
            List<QuoteRequests> lcs = QuoteRequests.GetAllQuoteRequests(conZP);
            int i = 0;
            foreach (QuoteRequests pro in lcs)
            {
                strRequests += @"<tr>   
                                <td>" + (i + 1) + @"</td>                                           
                                <td>" + pro.FirstName + @"</td>
                               ...
                               <td><a href='CompanyEmail:" + pro.EmailId + "'>" + pro.EmailId + @"</a></td>
                                <td>" + pro.ContactNo + @"</td> 
                                <td class='text-center'><a href='javascript:void(0);' data-bs-toggle='modal' data-bs-target='#fadeInRightModal' class=' btnmsg' data-id=" + pro.Id + @" data-name=" + pro.FirstName + @" Title='View More About Project'><i class='mdi mdi-email-outline h2 text-warning'></i></a></td>
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conZP = new SqlConnection(ConfigurationManager.ConnectionStrings["conZP"].ConnectionString);
            QuoteRequests BD = new QuoteRequests();
            BD.Id = Convert.ToInt32(id);
            int exec = QuoteRequests.DeleteQuoteRequests(conZP, BD);
            if (exec > 0)
            {
                x = "Success";
            }
    ...
    public static string GetMessage(string id)
    {
        ...
        {
            SqlConnection conZP = new SqlConnection(ConfigurationManager.ConnectionStrings["conZP"].ConnectionString);
            message = QuoteRequests.GetMessageById(conZP, id);
        }
        catch (Exception ex)
        {
