...
public partial class Admin_view_admin_users : System.Web.UI.Page
{
    ...
    public void GetAllUsers()
    {
        ...
        {
            strUsers = "";
            List<CreateUser> instrs = CreateUser.GetAllUser(conZP);
            int i = 0;
...
<div class='form-check form-switch markBlocked' id='chk_" + cat.Id + @"'  data-id='" + cat.Id + @"'>
  ...
  <td>" + chk + @"</td> 
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conZP = new SqlConnection(ConfigurationManager.ConnectionStrings["conZP"].ConnectionString);
            if (CreateUser.CheckAccess(conZP, "view-admin-users.aspx", "Delete", HttpContext.Current.Request.Cookies["zp_aid"].Value))
            {
                ...
                cat.AddedBy = HttpContext.Current.Request.Cookies["zp_aid"].Value;
                int exec = CreateUser.DeleteUser(conZP, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
    ...
    public static string BlockUsers(string id, string ftr)
    {
        ...
        {
            SqlConnection conZP = new SqlConnection(ConfigurationManager.ConnectionStrings["conZP"].ConnectionString);
            if (CreateUser.CheckAccess(conZP, "view-admin-users.aspx", "Edit", HttpContext.Current.Request.Cookies["zp_aid"].Value))
            {
                int exec = 0;
                CreateUser pro = new CreateUser();
                if (ftr == "Yes")
                {
                    exec = CreateUser.BlockUser(conZP, id, HttpContext.Current.Request.Cookies["zp_aid"].Value);
                    vali = "Blocked";
                }
                else
                {
                    exec = CreateUser.UnBlockUser(conZP, id, HttpContext.Current.Request.Cookies["zp_aid"].Value);
                    vali = "UnBlocked";
                }
