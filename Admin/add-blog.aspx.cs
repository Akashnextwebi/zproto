...
public partial class Admin_add_blog : System.Web.UI.Page
{
    ...
    public void GetBlogs()
    {
        try
        {
            BlogDetails BD = BlogDetails.GetAllBlogDetailsWithId(conZP, Convert.ToInt32(Request.QueryString["id"])).FirstOrDefault();
            if (BD != null)
            {
                divimg.Visible = true;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (Page.IsValid)
            {
                ...
                var aid = Request.Cookies["zp_aid"].Value;
                ...
                if (BD.ThumbImage != "" && BD.BlogImage != "")
                {
                    if (btnSave.Text == "Update")
                    {
                        ...
                        int result = BlogDetails.UpdateBlogDetails(conZP, BD);
                        if (result > 0)
                        {
                            GetBlogs();
                    ...
                    {
                        int result = BlogDetails.InsertBlogDetails(conZP, BD);
                        if (result > 0)
                        {
                            ...
                            txtDesc.Text = txtMetaDesc.Text = txtName.Text = txtUrl.Text = txtPageTitle.Text = txtMetaKey.Text = txtPostedOn.Text = txtPostedBy.Text="";
